...
// per‐line word budgets: line0=3 words, line1=2 words, line2=4 words
const uint8_t maxWordsLine[MAX_LINES] = { 2, 2, 1 };
...
// animated, word-by-word justified display with per-line budgets
void displayJustifiedAnimated(const char *msg,
  ...
  for (uint8_t shown = 1; shown <= wcount; shown++) {
    // compute wrapping given the first `shown` words
    ...
    for (uint8_t w = 0; w < shown; w++) {
      ...
      if (lineCount[ln] == 0) {
        ...
        // can we fit one more word by width AND by per-line count?
        if (lineCount[ln] + 1 <= maxWordsLine[ln]
            && curW + 2 + lw <= SCREEN_W) {
          curW += 2 + lw;
          lineCount[ln]++;
        }
...
void loop() {
  displayJustifiedAnimated(
    ...
    maxWordsLine        // our per-line word budgets
  );
