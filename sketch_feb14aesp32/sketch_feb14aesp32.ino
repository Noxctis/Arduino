/*******************************************************************
 * ESP32-C3 Super Mini: "Team Sync" PID Tuner & UART Bridge
 * UPGRADED: Echo-cancellation and bundled packet limits for 0 lag.
 *******************************************************************/
...
<head>
  ...
  <style>
    ...
    .highlight-box { background: #000; color: #0f0; border: 2px solid #007bff; }
  </style>
</head>
<body>
  ...
  <pre id="pids">Waiting for update...</pre>

  <h2>System State (Live)</h2>
  <pre id="sysState" class="highlight-box">Waiting for Teensy data...</pre>
...
<script>
  ...
  function updatePidBox() {
    let bs  = parseInt(document.getElementById('bs_val').value);
    ...
    let kec = parseFloat(document.getElementById('ke_c').value);
    let kef = parseFloat(document.getElementById('ke_f').value);

    let s = `Speed: ${bs}\n`;
    s += `Kp: ${(kpc + kpf).toFixed(2)}\n`;
    s += `Ki: ${(kic + kif).toFixed(2)}\n`;
    s += `Kd: ${(kdc + kdf).toFixed(2)}\n`;
    s += `Ke: ${(kec + kef).toFixed(2)}`;
  ...
  function sendPID() {
    ...
    // Throttle bumped to 100ms so you don't overwhelm the buffer while dragging
  ...
  function performSend() {
    ...
    let bs = document.getElementById('bs_val').value;
    let kp = (parseFloat(document.getElementById('kp_c').value) + parseFloat(document.getElementById('kp_f').value)).toFixed(2);
    let ki = (parseFloat(document.getElementById('ki_c').value) + parseFloat(document.getElementById('ki_f').value)).toFixed(2);
    let kd = (parseFloat(document.getElementById('kd_c').value) + parseFloat(document.getElementById('kd_f').value)).toFixed(2);
    let ke = (parseFloat(document.getElementById('ke_c').value) + parseFloat(document.getElementById('ke_f').value)).toFixed(2);
    
    if (ws && ws.readyState === WebSocket.OPEN) {
        // Send a SINGLE string containing everything to reduce Wi-Fi traffic
        ws.send(`ALL:${bs}|${kp}|${ki}|${kd}|${ke}`);
    }
  }
  ...
  window.onload = () => {
    ...
    sv.appendChild(makeSlider('bs_val','0','255','1','90','Base Speed'));
    sv.appendChild(makeSlider('kp_c','0','100','1','0','Kp (Coarse)'));
    sv.appendChild(makeSlider('kp_f','0','0.99','0.01','0.02','Kp (Fine)'));
    sv.appendChild(makeSlider('ki_c','0','100','1','0','Ki (Coarse)'));
    sv.appendChild(makeSlider('ki_f','0','0.99','0.01','0','Ki (Fine)'));
    sv.appendChild(makeSlider('kd_c','0','100','1','1','Kd (Coarse)'));
    sv.appendChild(makeSlider('kd_f','0','0.99','0.01','0','Kd (Fine)'));
    sv.appendChild(makeSlider('ke_c','0','20','1','2','Enc Kp (Coarse)'));
    sv.appendChild(makeSlider('ke_f','0','0.99','0.01','0','Enc Kp (Fine)'));
    ...
    ws.onmessage = e => {
      ...
      if(msg.indexOf('\t') !== -1) {
        ...
        document.getElementById('sysState').innerText = msg.substring(4).trim();
      }
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length){
  if(type == WStype_TEXT){
    ...
    // REMOVED THE BROADCAST ECHO HERE TO PREVENT SLIDER RUBBER-BANDING
  }
}
